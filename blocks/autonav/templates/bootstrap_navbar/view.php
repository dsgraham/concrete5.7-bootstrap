...
foreach ($navItems as $ni) {
  ...
  echo '<a href="' . (($ni->level == 1 && $ni->hasSubmenu) ? '#' : $ni->url) . '" target="' . $ni->target . '" class="' . $ni->classes . '';
  if ($ni->hasSubmenu) { echo ' dropdown-toggle" data-toggle="dropdown" aria-expanded="false"';}
  echo '">';
  if ($ni->level == 1) {
    $name = explode(' ', $ni->name);
    $last_word = array_pop($name);
    if (count($name)) {
      echo '<span>'.implode(' ', $name).'</span>';
      echo '<span>'.$last_word.'</span>';
    }
    else
      echo '<span class="single">'.$last_word.'</span>';
  } 
  else {
    echo $ni->name;
  }
  ...
  if ($ni->hasSubmenu) {
    ...
    //if (!$ni->cObj->getAttribute('replace_link_with_first_in_nav'))
    //  echo '<li class="parent"><a href="' . $ni->url . '" target="' . $ni->target . '">' . $ni->name . '</a>';
    //$sub_nav_count++;
  } else {
